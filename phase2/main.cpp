...
{
public:
  int length;//size of host's queue. MAY NEED 2 WILLCHECK
  queue<Packet> buffer;
...
//  double throughput; //number of bytes/total simulation time
  double T;//user-defined. Test different values and find the best one
//  bool busy;//Unnecessary, but could be used; intended for channel busy or not; 0 = idle(free) and 1 = busy
//  double avgnetdelay;//total delay of all hosts (dqueuing +dtransmission)/throughput
//  int retransmitcounter;//has three attempts to retransmit; after, discard frame
  double timeoutvalue;
  int numbytes = 0;
  double totaldelay;
...
//1 is ack, 0 is data
       ...
       if (currEvent.ackorData == 0)
       {
         ...
         else if (checkEvent.corrupt != 0 || checkEvent.corrupt != 1)
         {
           cout << "Data packet did not arrive correctly\n";
         }
...
/*//CHECK BELOW
        ...
        events.insertEvent(departure);/////////CHECKABOVE*/
...
//PROBABLY WRONG BELOW
            ...
            {
              cout << "error 6: data corruption check\n";
            }
         ...
         /*   if (timeout.time > currEvent.time + timeoutvalue)
            {
              hosts[currEvent.sender].ranBackoff = ceil(drand48() * hosts[currEvent.sender].attempts);
            }//SOMETHING HERE IS WRONG*/
          }
 /*         event channelsense;
          channelsense.receiver = currEvent.receiver;
          events.insertEvent(channelsense);*/
          //^Should also go in above else if or no?
      ...
      else if (currEvent.check == 1)//don't check
      {
        ...
        else
        {
          ...
          {
            cout << "error 9: ack packet departure check2\n";
          }
          else
          {
            cout << "error8: ack packet departure check\n";
          }
        }
      }
